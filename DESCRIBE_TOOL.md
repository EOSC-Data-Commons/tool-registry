...
- `types`: List of types of the tool, which can be used to categorize the tool based on its functionality or domain. This can be your choice or use one of the predefined archetypes ["galaxy_workflow", "vip_app_boutique"]
...
- `input_slots`: A list of input slots that the tool can accept. This identifies the number of inputs for the tool as `slots`. The JSON object has this schema
...
- `output_slots`: A list of output slots that the tool can produce. This identifies the number of outputs for the tool as `slots`. The JSON object has this schema
...
- `raw_definition`: A field to store the raw definition of the tool. This can be used to store the original definition of the tool as it was provided by the user or harvest source. E.g. Galaxy workflow definition.
